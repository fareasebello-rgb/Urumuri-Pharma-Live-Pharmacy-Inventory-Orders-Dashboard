/* ===========================================================
   ...
   Tries the FastAPI backend first; if it isn't running, falls back
   to the static data.json snapshot (which has both time windows
   precomputed) so this still works when you just open index.html.
   =========================================================== */
...
// Chart instances, kept so we can destroy + redraw when the time range changes.
const charts = {};

// Cached offline snapshot + cached API availability, so toggling the
// time range doesn't refetch from scratch every time.
let snapshotCache = null;
let useLiveApi = false;
let currentInventory = []; // for the search filter to work against
...
/* ---------------------------------------------------------
   Data loading: tries the live API for the given window,
   falls back to the precomputed snapshot's windows[months].
   --------------------------------------------------------- */
...
async function detectApiAndCacheSnapshot() {
  // Always load the offline snapshot as a safety net.
...
function renderInventoryTable(rows) {
  ...
  empty.hidden = true;
  // Already sorted by urgency from the source — never re-sort alphabetically here.
  tbody.innerHTML = rows.map(r => `
    <tr>
      <td>${r.product}</td>
      <td>${r.category}</td>
...
(async function init() {
  ...
  try {
    ...
    console.error("Failed to render new orders:", err);
  }
