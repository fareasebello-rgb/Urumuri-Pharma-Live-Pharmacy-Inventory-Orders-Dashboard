...
insights.py
-----------
Turns raw sales + inventory rows into plain-language takeaways.
Kept separate from app.py so the "data analysis" logic (the part that
actually decides what's worth saying about the numbers) is easy to
find, read, and extend on its own.

This mirrors exactly what data_generator.py precomputes for the
offline snapshot — so online (via the API) and offline (via data.json)
modes say the same kinds of things.
"""
...
def compute_insights(monthly: dict, category_totals: dict, region_totals: dict,
    ...
    """
    monthly: {"2025-01": 11546600, ...}
    category_totals / region_totals: {"Antibiotics": 18532300, ...}
    inventory_with_velocity: rows from app.py's inventory() with
        needs_reorder / days_of_stock_remaining already attached.
    """
...
def get_cutoff_date(max_date_str: str | None, months: int | None):
    """First day of the month that is `months` months before the latest
    date in the dataset. Returns None if months is falsy (= no filter)."""
